            ...
            steps {
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-key']]) {
                        ...
                        helm install kafka bitnami/kafka --set persistence.storageClass=ebs-sc --set persistence.size=8Gi --set zookeeper.enabled=false --set externalZookeeper.servers=zookeeper.default.svc.cluster.local:2181 --set replicaCount=3 --set controller.replicaCount=0
                        '''
                    }
                }
            }
